using Efarming_Sustainability.App.Infraestructure.API;
using Efarming_Sustainability.App.Infraestructure.Database;
using Efarming_Sustainability.Core.Models;
using System;
using System.Collections.Generic;
using System.Net.Http;
using System.Threading.Tasks;

namespace Efarming_Sustainability.App.Infraestructure.Repository.SQLite
{
    public class FarmStatusRepository : SQLiteConnection
    {
        private readonly HttpClient _httpClient;

        public FarmStatusRepository()
        {
            _httpClient = new HttpClient();
        }

        public async Task InitializeAsync()
        {
            await InitAsync();
            await _db.CreateTableAsync<FarmStatus>();
        }

        public async Task<List<FarmStatus>> GetFarmStatuses()
        {
            var api = new ConsumoApi("https://localhost:7292/api/FarmStatuses/");
            try
            {
                var result = await api.GetAsync<List<FarmStatus>>("getFarmStatuses");
                return result;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error al obtener los estados de finca: {ex.Message}");
                return new List<FarmStatus>();
            }
        }

        public async Task SaveFarmStatusesLocally(List<FarmStatus> statuses)
        {
            await InitializeAsync();
            await _db.DeleteAllAsync<FarmStatus>();
            await _db.InsertAllAsync(statuses);
        }

        public async Task<List<FarmStatus>> GetLocalFarmStatuses()
        {
            await InitializeAsync();
            return await _db.Table<FarmStatus>().ToListAsync();
        }

        public async Task DeleteAllFarmStatuses()
        {
            await InitializeAsync();
            await _db.DeleteAllAsync<FarmStatus>();
        }
    }
}